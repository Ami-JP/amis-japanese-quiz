...
function sortRowsByKanjiOrder(rows: KanjiHintRow[], kanjis: string[]) {
  ...
  for (const row of rows) {
    if (!rowMap.has(row.kanji)) rowMap.set(row.kanji, row);
  }
  return kanjis.map((kanji) => rowMap.get(kanji)).filter((row): row is KanjiHintRow => Boolean(row));
}
...
function makeQuizItems(sourceRows: KanjiHintRow[], pool: KanjiHintRow[]) {
  return sourceRows
    ...
    .map((q) => {
      ...
      const noSharedTagCandidates = baseCandidates.filter((item) => !hasSharedTags(currentTags, parseTags(item.tags)));

      if (noSharedTagCandidates.length >= 3) {
        narrowedCandidates = noSharedTagCandidates;
      } else if (q.school_grade) {
        const sameGrade = baseCandidates.filter((item) => item.school_grade === q.school_grade);
        const sameGradeNoShared = sameGrade.filter((item) => !hasSharedTags(currentTags, parseTags(item.tags)));
        if (sameGradeNoShared.length >= 3) narrowedCandidates = sameGradeNoShared;
        else if (sameGrade.length >= 3) narrowedCandidates = sameGrade;
      } else if (q.jlpt_level) {
        const sameJlpt = baseCandidates.filter((item) => item.jlpt_level === q.jlpt_level);
        const sameJlptNoShared = sameJlpt.filter((item) => !hasSharedTags(currentTags, parseTags(item.tags)));
        if (sameJlptNoShared.length >= 3) narrowedCandidates = sameJlptNoShared;
        else if (sameJlpt.length >= 3) narrowedCandidates = sameJlpt;
      }

      const uniqueWrongLabels = Array.from(new Set(narrowedCandidates.map((item) => buildMeaningLabel(item)).filter(Boolean)));
      ...
      return {
        ...
        questionText: "Which of the following is closest in meaning to this kanji?",
...
async function fetchKanjiPool(db: any, unit?: string) {
  let query = db
    .from("kanji_hints")
    .select("kanji, meaning_ja, meaning_en, school_grade, jlpt_level, unit, order_in_unit, tags")
...
async function getCurrentProgress(db: any, accountId: string, unit: string) {
  const { data, error } = await db
    ...
    .select("student_account_id, unit, last_order_completed, last_studied_at, is_completed")
...
async function moveToNextUnitIfNeeded(params: { supabase: any; accountId: string; currentUnit: string; newLastOrderCompleted: number; }) {
  ...
  if ((remainingCount ?? 0) > 0) return { nextUnit: null };
  ...
  const { error: completeProgressError } = await db.from("student_kanji_progress").upsert(completeProgressRow);
  if (completeProgressError) throw new Error(completeProgressError.message);
  ...
  const { data: nextUnitRowRaw, error: nextUnitError } = await db
    ...
    .limit(1)
    .maybeSingle();
  if (nextUnitError) throw new Error(nextUnitError.message);

  const nextUnitRow = nextUnitRowRaw as { unit: string | null } | null;
  if (!nextUnitRow?.unit) return { nextUnit: null };

  const nextUnit = nextUnitRow.unit;
  const { error: updateAccountError } = await db.from("student_accounts").update({ current_unit: nextUnit }).eq("id", accountId);
  ...
  const { error: nextProgressError } = await db.from("student_kanji_progress").upsert(nextProgressRow);
...
function getLatestWrongKanji(attempts: AttemptHistoryRow[]) {
  const latestByKanji = new Map<string, { kanji: string; order_in_unit: number | null; is_correct: boolean }>();
  for (const row of attempts) {
    if (!latestByKanji.has(row.kanji)) {
      latestByKanji.set(row.kanji, { kanji: row.kanji, order_in_unit: row.order_in_unit, is_correct: row.is_correct });
    }
  }
  return Array.from(latestByKanji.values()).filter((row) => row.is_correct === false).map((row) => row.kanji);
}
...
async function getLoggedInAccount(db: any) {
  ...
  if (!session?.studentAccountId) {
    return {
      account: null,
      errorResponse: NextResponse.json({ error: "Unauthorized. Please log in again." }, { status: 401 }),
    };
  }
  ...
  if (accountError || !account) {
    return {
      account: null,
      errorResponse: NextResponse.json({ error: accountError?.message ?? "Account not found." }, { status: 400 }),
    };
  }
  ...
  if (!account.is_active) {
    return {
      account: null,
      errorResponse: NextResponse.json({ error: "This account is inactive." }, { status: 403 }),
    };
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const mode = (request.nextUrl.searchParams.get("mode") as "normal" | "review-wrong" | "review-studied") ?? "normal";
    ...
    if (mode === "review-wrong" || mode === "review-studied") {
      ...
      if (attemptsError) return NextResponse.json({ error: attemptsError.message }, { status: 400 });

      const attempts = (attemptsRaw ?? []) as AttemptHistoryRow[];
      const targetKanjis = mode === "review-wrong"
        ? getLatestWrongKanji(attempts).slice(0, REVIEW_LIMIT)
        : uniquePreserveOrder(attempts.map((row) => row.kanji)).slice(0, REVIEW_LIMIT);

      if (targetKanjis.length === 0) {
        const progress = account.current_unit ? await getCurrentProgress(db, account.id, account.current_unit) : null;
      ...
      const sourceRows = sortRowsByKanjiOrder(globalPool, targetKanjis).slice(0, REVIEW_LIMIT);
      const progress = account.current_unit ? await getCurrentProgress(db, account.id, account.current_unit) : null;
    ...
    if (!activeUnit) return NextResponse.json({ error: "current_unit is empty." }, { status: 400 });

    let activePool = await fetchKanjiPool(db, activeUnit);
    let progress = await getCurrentProgress(db, account.id, activeUnit);
    let activeLastOrderCompleted = startFromBeginning ? 0 : progress?.last_order_completed ?? 0;
    ...
    if (orderedRows.length === 0 && !lockedToUnit) {
      const { data: nextUnitRowRaw, error: nextUnitError } = await db
        ...
        .limit(1)
        .maybeSingle();
      if (nextUnitError) return NextResponse.json({ error: nextUnitError.message }, { status: 400 });

      const nextUnitRow = nextUnitRowRaw as { unit: string | null } | null;
      if (nextUnitRow?.unit) {
        activeUnit = nextUnitRow.unit;
        const { error: updateCurrentUnitError } = await db.from("student_accounts").update({ current_unit: activeUnit }).eq("id", account.id);
        if (updateCurrentUnitError) return NextResponse.json({ error: updateCurrentUnitError.message }, { status: 400 });

        const nextProgressRow = { student_account_id: account.id, unit: activeUnit, last_order_completed: 0, last_studied_at: null, is_completed: false };
        const { error: upsertNextProgressError } = await db.from("student_kanji_progress").upsert(nextProgressRow);
        if (upsertNextProgressError) return NextResponse.json({ error: upsertNextProgressError.message }, { status: 400 });

        activePool = await fetchKanjiPool(db, activeUnit);
        orderedRows = activePool
          .filter((row) => row.order_in_unit !== null && row.order_in_unit > 0)
          .sort((a, b) => (a.order_in_unit ?? 0) - (b.order_in_unit ?? 0))
          .slice(0, QUESTION_LIMIT);
      }
    }
    ...
    return NextResponse.json({
      account: { display_name: account.display_name, student_login_id: account.student_login_id },
      unit: activeUnit,
      lastOrderCompleted: startFromBeginning ? 0 : progress?.last_order_completed ?? 0,
      mode,
      lockedToUnit,
      finished: sourceRows.length === 0,
      isUnitComplete: sourceRows.length === 0 && !startFromBeginning,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const mode = (body.mode as "normal" | "review-wrong" | "review-studied") ?? "normal";
    ...
    if (mode !== "normal") return NextResponse.json({ ok: true });
    ...
    const currentLastOrderCompleted = startFromBeginning ? 0 : progress?.last_order_completed ?? 0;
    const maxAttemptOrder = getMaxAttemptOrder(attempts);

    const newLastOrderCompleted = maxAttemptOrder > 0
    ...
    const { error: progressError } = await db.from("student_kanji_progress").upsert(progressRow);
    ...
    const { count: remainingCount, error: remainingError } = await db
      ...
      .eq("is_published", true)
      .gt("order_in_unit", newLastOrderCompleted);
    if (remainingError) return NextResponse.json({ error: remainingError.message }, { status: 400 });
    ...
    if (!hasRemaining) {
      ...
      const { error: completeError } = await db.from("student_kanji_progress").upsert(completeRow);
      if (completeError) return NextResponse.json({ error: completeError.message }, { status: 400 });

      if (!lockToUnit) {
        await moveToNextUnitIfNeeded({ supabase, accountId: account.id, currentUnit: unit, newLastOrderCompleted });
      }
    }
